...
const upperSnakeCase = (it?: string) => snakeCase(it)?.toUpperCase()
...
export class BetterError extends Error {
  ...
  public code: string
  public cause: Error | BetterError | (Error | BetterError)[] | null | undefined
  public info: any
    ...
    return Object.keys(this)
      ...
      .reduce((accum, key) => {
        if (key === 'cause') {
          ...
          accum.cause = omittings.includes(key)
            ? BetterError.OMISSION
...
function _deriveCodeFromName(name: string) {
  ...
  return `E_${upperSnakeCase(name)}`.replace(/_ERROR$/, '')
}
